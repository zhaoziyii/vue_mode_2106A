<template>
  <div class="login">
    ...
    <van-cell-group inset class="form">
      ...
      <van-field
        v-model="data.loginParams.userPass"
    ...
    <div class="tishi">
      <p>忘记密码?</p>
      <p class="zc" @click="toLogin">注册/验证码登录</p>
    </div>
  </div>
</template>
...
onMounted(() => {
  getImgyzm();
  data.loginParams.userPass = route.query.user_pass;
});
console.log(data.loginParams.userPass);

/* 图形验证码 */
const getImgyzm = async () => {
...
const Gologin = async () => {
  const res = await proxy.$API.getLoginApi({
    ...
    user_pass: data.loginParams.userPass,
    device_type: "2",
    device_id: "1",
  });
